...
class Utilisateur extends CI_Controller
{
	...
	public function list(){
		...
		else
		{
			...
			$this->template->set('title','Covid19 - '.$this->lang->line('users').'');
			$this->template->load($this->templates,'admin/listUtilisateur',$this->data);
		}
	}
	...
	public function add()
	{
		...
		{*/
			...
			if ($this->form_validation->run() === TRUE)
			{
				...
				$grpuser  =  $this->input->post('groups');
				if (isset($grpuser) && $grpuser != NULL) {
					$groups = $grpuser;
				}
				else{
					$groups = 5;
			...
			else
			{
				...
				$this->data['groupsexist'] = $this->ion_auth->groups()->result_array();	   
